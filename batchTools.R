...
doBatchRun <- function( arg )
{
  ...
  fitSISCA(ctlFile=arg[1], folder=arg[2])
  return(NULL)
}
                          ...
                          folderLabels = FALSE )
{
  ...
  { 
    ...
    if ( file.exists( batchParFile[i] ) )
    {
      ...
      if( folderLabels )
      {
        folderName <- paste(batchDesign$scenarioLabel[i],"_",batchDesign$mpLabel[i],sep = "")
        folderName <- gsub("'","",folderName)
      } else 
        folderName <- paste(prefix,"bat",i,sep ="")

     
      fitSISCA( batchParFile[i], folder = folderName )
...
.createBatchDesign <- function( ctlPar, basePars, prefix="job" )
{
  ...
  for ( i in 1:nScenario )
  {
    ...
    for ( j in 1:nMP )
    {
      ...
      scenarioLabel[iBatch] <- newPars[,2][ newPars[,1]=="ctrl$dataScenarioName" ]
      mpLabel[iBatch] <- newPars[,2][ newPars[,1]=="ctrl$modelHypName" ]
